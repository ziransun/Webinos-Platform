      ...
      if(connPzh.authorized) {
        ...
        try {
          if(self.parent.config.otherCert.hasOwnProperty(to)) {
            ...
            {
              var services = self.parent.rpcHandler.getAllServices();
          ...
          session.common.readJson(self, buffer, function(obj) {
            session.common.processedMsg(self, obj, function(validMsgObj) {
              ...
              if(validMsgObj.type === 'prop' && validMsgObj.payload.status === 'foundServices') {
                ...
                self.parent.rpcHandler.addRemoteServiceObjects(validMsgObj.payload.message);
                ...
                var services = self.parent.rpcHandler.getAllServices(validMsgObj.from);
...
PzhConnecting.prototype.sendCertificate = function(to, callback) {
  ...
  options.port   = session.configuration.httpServerPort;
  ...
  var payload = {cmd: "sendCert" , name: self.parent.config.serverName, to: to, cert: self.parent.config.master.cert, crl: self.parent.config.master.crl};
