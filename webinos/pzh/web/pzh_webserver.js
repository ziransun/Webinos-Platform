...
pzhWebInterface.start = function(hostname, callback) {
  createWebInterfaceCertificate(farm.config, function(webServer){
    var server = https.createServer(webServer, function(req, res){
      var parsed = url.parse(req.url), query = {}, filename;
      if (parsed.query){
        query = querystr.parse(parsed.query);
      } 

      if (query && query.id === "verify"){
        fetchOpenIdDetails(req, res, function(provider, userid) {// Important step as we assign pzh instance
          // redirection to same page but without details fetched from google.
          res.writeHead(302, {Location: '/main.html?provider='+provider+'?id='+userid}); 
          res.end();
        });
      } else {
        currentPzh = query.id;
        var pzhName = parsed.pathname;
        for(var key in farm.pzhs) {
          if (key.split('/')[1] === pzhName.split('/')[1]) {
            filename = path.join(__dirname, "addConnection.html");
          }
        }
    ...
    var httpsServer = https.createServer(webServer, function(request, response) {
      var parsed = url.parse(request.url), query = {};
      if (parsed.query) {
        query = querystr.parse(parsed.query);
        if (query.cmd === "transferCert") {
          request.on('data', function(data) {
            var parse = JSON.parse(data);
            if (parse.cmd === "sendCert" && farm.pzhs[parse.to]) {
              receiveCertificate(farm.pzhs[parse.to], parse);
              var payload = {cmd: "receiveCert", cert: farm.pzhs[parse.to].config.master.cert, crl: farm.pzhs[parse.to].config.master.crl};
        ...
        if (parse.cmd && parse.from) {
          switch(parse.cmd) {
            case 'listDevices':
              ...
              break;
            case 'userDetails':
              var payload = {email:  pzh[parse.from].config.email, 
                    country: pzh[parse.from].config.country, 
                    image:   pzh[parse.from].config.image,
                    name:    pzh[parse.from].config.name,
                    servername: pzh[parse.from].config.serverName};
              result({cmd:'userDetails', payload: payload});
              break;
            case 'pzhPzh':
