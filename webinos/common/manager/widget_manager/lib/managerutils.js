...
this.ManagerUtils = (function () {
    ...
    */
    ManagerUtils.mkdirs = function (dirPath) {
        ...
        if (process.platform == 'win32') dirPath = (dirPath[1] == ':') ? dirPath.substr(0, 3) : '';
        else dirPath = (dirPath[0] == '/') ? '/' : '';
        ...
        while (pathElements.length) {
            ...
            if (nPathV.existsSync(dirPath)) {
                ...
                if (stat.isFile()) {
                    throw new Error('ManagerUtils.mkdirs: specified path is a file: ' + dirPath);
                }
                if (stat.isDirectory())
                    continue;
            } else {
